            ...
            for row in 0...model.rowCount(rootIndex()) do
                ...
                if value > 0.0
                    ...
                    if selections.isSelected(labelIndex)
                      s |= Qt::Style::State_Selected.to_i
                      STDERR.puts "cannot set 'selected' state, qtruby BUG"
                    end
                    if currentIndex() == labelIndex
                      s |= Qt::Style::State_HasFocus.to_i
                      STDERR.puts "cannot set 'has focus' state, qtruby BUG"
                    end
                    state = Qt::Style::State_Selected # it does not matter
                    state.value = s
#                     tag "s = #{s.inspect}, state = #{state.inspect}, #{state.class}"
#                     option.state = s.to_i # state          DOES NOT LIKE IT ???
                        # complains: undefined methode state= for Qt;:StyleOptionViewItem.
                        # it does not like int and also Qt::Enum ...
#                     option.state = option.state       BOOM TOO....
...
Reform::createInstantiator File.basename(__FILE__, '.rb'), QPieView, Reform::PieView
