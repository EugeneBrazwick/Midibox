...
# - each. Chunks can be treated as eventsource
...
#   it returns the event with the smallest timestamp and priority
...
# A chunk can also be used as a consumer.
    class Chunk < Producer
      ...
      private
        ...
        # [options] valid keys are:
        #           [:split_channels] to record each channel on its own track. This does not merge tracks,
        #                            even if they share a channel
        #           [:split_tracks] same as +split_channels
        #           [:combine_notes] Track option, create NoteEvent from NoteOn + NoteOff
        #           [:combine_progchanges] Track option, create extended program changes that include the bank selection
        #           [:combine_lsb_msb] Track option, create control14 events (but just as ControllerEvent)
        #           [:tmpltrack] Track option, create a split track from a source track. Copies stuff like
        #                        the copyright notice etc..
        #           [:template] same as +tmpltrack+
        #
        # A track is only created when the first event is send
...
#           tag "#{self}.consume, producer = #{producer}"
